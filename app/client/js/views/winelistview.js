...
(function (Weinstein, Marionette, Handlebars) {
	...
	var WINE_TEMPLATE = '' +
		'<td class="text-center">{{#if nr}}{{nr}}{{else}}-{{/if}}</td>' +
                '<td>{{applicant.label}} {{applicant.lastname}}</td>' +
                '<td>{{applicant.association.name}}</td>' +
                '<td>{{ label }}</td>' +
                '<td>{{ winesort.name }}</td>' +
                '<td>{{ vintage }}</td>' +
	...
	/**
	 * @class Weinstein.View.WineListView
	 */
	...
	Weinstein.Views.WineListView = WineListView;
