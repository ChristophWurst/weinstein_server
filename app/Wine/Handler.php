        ...
        try {
            ...
            foreach ($sheet->toArray() as $row) {
                ...
                if (is_null($wine)) {
                    ...
                    throw new ValidationException(new MessageBag(['Wein '.$row[0].' nicht vorhanden']));
                }
                $this->updateKdb($wine, [
                    'value' => true,
                ]);
        ...
        try {
            ...
            foreach ($sheet->toArray() as $row) {
                ...
                if (! $wine->kdb) {
                    ...
                    throw new ValidationException(new MessageBag(['Wein '.$row[0].' ist kein KdB']));
                }
                $this->updateSosi($wine, [
                    'value' => true,
                ]);
                $rowCount++;
            }
