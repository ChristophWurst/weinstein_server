...
class CatalogueHandler implements TastingCatalogueHandler
{
    ...
    private function assignCatalgueNumberToWine(Competition $competition, int $wineNr, int $tastingNumber)
    {
        ...
        if (is_null($wine)) {
            ...
            throw new ValidationException(new MessageBag(['Wein '.$wineNr.' nicht vorhanden']));
        }
        if (! $wine->chosen) {
            throw new ValidationException(new MessageBag(['Nur ausgeschenkte Weine werden in den Katalog aufgenommen']));
        }
